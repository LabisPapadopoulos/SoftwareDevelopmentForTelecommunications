...
public class Gui implements Runnable {
    ...
    private final JComboBox<String> androidDevices;
    ...
    private static JLabel manufacturer;
    private static JLabel version;
    private static JLabel latitude;
    private static JLabel longitude;
    private static JLabel altitude;
    private static JLabel level;
    private static JLabel charging;
    
    private SwingWorker<Object, List<String>> swingWorker;
    private SwingWorker<Object, List<String>> androidSwingWorker;
    ...
    public Gui(Manager manager, long guiRefreshTime) throws ClassNotFoundException, SQLException {
        ...
        androidDevices = new JComboBox<String>();
        ...
        //gia na gemizei to tab tou android
        androidSwingWorker = new SwingWorker<Object, List<String>>() {
            ...
            protected Object doInBackground() throws Exception {
                while(!Thread.currentThread().isInterrupted()) {
                    ...
                    //tin process
                    //to this tis ap' exw klashs, oxi tou SwingWorker
                    publish(Gui.this.manager.getAndroidDevices());
                    Thread.sleep(Gui.this.guiRefreshTime);
                }
                return null;
            }
            ...
            protected void process(List<List<String>> devicesHistory) {
                ...
                ActionListener[] actionListeners = androidDevices.getActionListeners();
                
                for(ActionListener actionListener : actionListeners) {
                    androidDevices.removeActionListener(actionListener);
                }
                
                String selected = (String)androidDevices.getSelectedItem();
                androidDevices.removeAllItems();

                for(String device : devicesHistory.get(devicesHistory.size() - 1)) {
                    androidDevices.addItem(device);
                }

                androidDevices.setEnabled((androidDevices.getItemCount() > 1));
                
                for(ActionListener actionListener : actionListeners) {
                    androidDevices.addActionListener(actionListener);
                }
                
                if(selected != null)
                    androidDevices.setSelectedItem(selected);
            } 
        };
    }
    ...
    public void run() {
        ...
        JPanel androidDeviceJPanel = new JPanel();
        androidDeviceJPanel.setLayout(new BoxLayout(androidDeviceJPanel, BoxLayout.Y_AXIS));
        
        JPanel topAndroidDevicePanel = new JPanel(new FlowLayout(FlowLayout.LEFT));
        topAndroidDevicePanel.add(new JLabel("Available Android Devices:"));

        topAndroidDevicePanel.add(androidDevices);
        androidDeviceJPanel.add(topAndroidDevicePanel);
        
        //Dhmiourgia tou Grid
        JPanel androidDeviceGrid = new JPanel();
        androidDeviceGrid.setLayout(new GridLayout(0, 2));
        
        androidDeviceGrid.add(new JLabel("Model:"));
        androidDeviceGrid.add(model);
        
        androidDeviceGrid.add(new JLabel("Manufacturer:"));
        androidDeviceGrid.add(manufacturer);
        
        androidDeviceGrid.add(new JLabel("Version:"));
        androidDeviceGrid.add(version);
        
        androidDeviceGrid.add(new JLabel("Latitude:"));
        androidDeviceGrid.add(latitude);
        
        androidDeviceGrid.add(new JLabel("Longitude:"));
        androidDeviceGrid.add(longitude);
        
        androidDeviceGrid.add(new JLabel("Altitude:"));
        androidDeviceGrid.add(altitude);
        
        androidDeviceGrid.add(new JLabel("Level:"));
        androidDeviceGrid.add(level);
        
        androidDeviceGrid.add(new JLabel("Charging:"));
        androidDeviceGrid.add(charging);
        
        androidDeviceJPanel.add(androidDeviceGrid);
        jTabbedPane.addTab("Android Devices", androidDeviceJPanel);
        //----------------------------telos 3ou tab---------------------------------
        ...
        androidDevices.addActionListener(new ActionListener() {

            @Override
            public void actionPerformed(ActionEvent e) {
                String androidId = (String)androidDevices.getSelectedItem();
                try {
                    AndroidDevice androidDevice = manager.getAndroidDevice(androidId);
                    
                    model.setText(androidDevice.getModel());
                    manufacturer.setText(androidDevice.getManufacturer());
                    version.setText(androidDevice.getVersion());
                    latitude.setText(Double.toString(androidDevice.getLatitude()));
                    longitude.setText(Double.toString(androidDevice.getLongitude()));
                    altitude.setText(Double.toString(androidDevice.getAltitude()));
                    level.setText(Float.toString(androidDevice.getLevel()) + " %");
                    charging.setText(androidDevice.getCharging());
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            gui.androidSwingWorker.execute();
